...
const SelectedComponentDetails: React.FC<SelectedComponentDetailProps> = React.memo(
  ({ buildHierarchyTree, setAiSummary, setLoadingSummary, loadingSummary, hookDetails }) => {
    ...
    const selectedMetrics = useMemo(
      () => (componentId ? allMetrics[componentId] : null),
      [componentId, allMetrics]
    );
      ...
      <div className="rounded-xl text-black shadow-md p-6 border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 dark:text-gray-100 flex-auto">
        ...
        {selectedMetrics ? (
          <div className="space-y-2 text-sm">
            ...
            <p><strong className="font-medium w-32">Path:</strong> {findPathInTree(buildHierarchyTree, selectedMetrics.id) || 'N/A'}</p>
            <p><strong className="font-medium w-32">Parent ID:</strong> {selectedMetrics.parentId || 'None'}</p>
            ...
            <p><strong className="font-medium w-32">Total Render Duration:</strong> {selectedMetrics.totalRenderDuration.toFixed(2)} ms</p>
